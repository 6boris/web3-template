...
import (
	...
	dexMember "web3/api/service/dex-finance"
...
func NewGRPCServer(c *conf.Bootstrap, s *service.Service, logger log.Logger) *grpc.Server {
	...
	if c.Server.Grpc.Addr != "" {
		...
		opts = append(opts, grpc.Address(fmt.Sprintf("0.0.0.0:%d", net.GetRandPort(61001, 62000))))
	}
	if c.Server.Grpc.Timeout != nil {
		opts = append(opts, grpc.Timeout(c.Server.Grpc.Timeout.AsDuration()))
	}
	srv := grpc.NewServer(opts...)
	dexMember.RegisterCoinServer(srv, s)
	dexMember.RegisterAccountServer(srv, s)
	return srv
}
