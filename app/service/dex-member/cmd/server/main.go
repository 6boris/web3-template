...
import (
	...
	"web3/app/service/dex-member/internal/server"
	"web3/app/service/dex-member/internal/service"
	"web3/pkg/app"
)
...
func newApp(conf *conf.Bootstrap, logger log.Logger) *kratos.App {
	...
	opts := app.GetAppOpts(app.APP_SERVICE_DEX_MEMBER)
	opts = append(opts,
		kratos.Server(
			grpcServer,
			httpServer,
...
func main() {
	...
	dao.New(bc)
	logger := app.GetAppLogger(app.APP_SERVICE_DEX_MEMBER)
