...
import (
	...
	"os"
	"web3/app/job/datawatch/internal/client"
	"web3/app/job/datawatch/internal/conf"
	"web3/app/job/datawatch/internal/dao"
	"web3/app/job/datawatch/internal/server"
...
func newApp(conf *conf.Bootstrap, logger log.Logger) *kratos.App {
	...
	taskServer := server.NewTASKServer(conf, svc, logger)
...
func main() {
	...
	flagConf = "app/job/datawatch/configs"
	flag.Parse()
	conf.Init(flagConf)
	bc := conf.Conf
